...
def create_cursor(cnx):
    #Criaremos um cursor, que é um objeto utilizado para executar instruções SQL no contexto do Python.
    cursor = cnx.cursor()
    return cursor
...
def read_csv(path):
    #inserindo dados da tabela num arquivo CSV
    df = pd.read_csv(path)
    return df

def add_product_data(cnx, cursor, df, db_name, tb_name):
    #inserindo dados do CSV nas tabelas
...
if __name__ == "__main__":
    ...
    cnx = connect_mysql("localhost", "Usuário", "12345")
